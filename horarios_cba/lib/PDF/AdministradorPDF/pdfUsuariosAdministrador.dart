...
class PdfUsuariosAdministradorScreen extends StatefulWidget {
  const PdfUsuariosAdministradorScreen({
    super.key,
  ...
  @override
  State<PdfUsuariosAdministradorScreen> createState() =>
      _PdfUsuariosAdministradorScreenState(
          usuario: usuario, registros: registros);
}

class _PdfUsuariosAdministradorScreenState
    extends State<PdfUsuariosAdministradorScreen> {
  final UsuarioModel usuario;
  ...
  Future<Uint8List> generatePdf(final PdfPageFormat format) async {
    final doc = pw.Document(
      title: 'CBA Mosquera reporte de usuarios',
    );
    ...
    final tableData = registros.map((item) {
      ...
      return cellsWithIndex.where((entry) {
        return entry.key < cellsWithIndex.length - 3;
      }).map((entry) {

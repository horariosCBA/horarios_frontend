...
class PdfResultadosAdministradorScreen extends StatefulWidget {
  const PdfResultadosAdministradorScreen({
    super.key,
  ...
  @override
  State<PdfResultadosAdministradorScreen> createState() =>
      _PdfResultadosAdministradorScreenState(
          usuario: usuario, registros: registros);
}

class _PdfResultadosAdministradorScreenState
    extends State<PdfResultadosAdministradorScreen> {
  final UsuarioModel usuario;
  ...
  Future<Uint8List> generatePdf(final PdfPageFormat format) async {
    final doc = pw.Document(
      title: 'CBA Mosquera reporte de resultados de aprendizaje',
    );
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (final context) => [
          ...
          pw.Center(
            child: pw.Text(
              "Reporte de resultados de aprendizaje",
              ...
              ...tableData.map((row) {
                return pw.TableRow(
                  children: row.asMap().entries.map((entry) {
                    final index =
                        entry.key; // Aquí obtienes el índice de la celda
                    final cell =
                        entry.value; // Aquí obtienes el valor de la celda

                    // Ajusta el ancho del contenedor dependiendo del índice
                    return pw.Container(
                      padding: const pw.EdgeInsets.all(5),
                      alignment: pw.Alignment.center,
                      width: index == 2 && index == 4
                          ? 150
                          : 100, 
                      child: pw.Text(
                        cell,

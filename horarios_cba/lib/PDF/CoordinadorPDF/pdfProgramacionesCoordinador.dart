...
class PdfProgramacionesCoordinadorScreen extends StatefulWidget {
  const PdfProgramacionesCoordinadorScreen({
    super.key,
  ...
  @override
  State<PdfProgramacionesCoordinadorScreen> createState() =>
      _PdfProgramacionesCoordinadorScreenState(
          usuario: usuario, registros: registros);
}

class _PdfProgramacionesCoordinadorScreenState
    extends State<PdfProgramacionesCoordinadorScreen> {
  final UsuarioModel usuario;
  ...
  Future<Uint8List> generatePdf(final PdfPageFormat format) async {
    final doc = pw.Document(
      title: 'CBA Mosquera reporte de programaciones',
    );
    ...
    final tableData = registros.map((item) {
      ...
      return cellsWithIndex.where((entry) {
        return entry.key < cellsWithIndex.length - 2;
      }).map((entry) {
